...
innovations_mv <- function(
  ...
  if (do_tol_stop) {lag_tol <- as.numeric(lag_tol)}
...
innovations_mv_grad <- function(
  ...
  if (max_n >= 1L && Lcurr >= 1L) {
    for (n in 1:max_n) {
      ...
      # tol rule on boundary lag (oldest retained) extremely small, fix Lcurr
...
aop1_score_matrix <- function(par, Xw, DesignX,
  ...
  fitg <- innovations_mv_grad(kappa = kappa_fun,
                              ...
                              lag_tol = NULL)   # !!! do NOT re-trigger tol logic !!! derivatives no lag_tol
