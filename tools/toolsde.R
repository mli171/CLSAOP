...
innovations_mv <- function(
  ...
  lag_cap <- if (is.null(lag_max)) max_n else {
  ...
  if (max_n >= 1L && lag_cap >= 1L) {
    for (n in 1:lag_cap) {

      Theta_n <- vector("list", n)
      ...
      mylag <- n
      if (do_tol_stop && max(abs(Theta_n[[n]])) < lag_tol) break
    }
  }

  if (mylag < max_n) {
    Theta_fixed <- Theta[[mylag + 1L]]
    V_fixed     <- V[[mylag + 1L]]
    Vinv_fixed  <- Vinv[[mylag + 1L]]
    for (tt in (mylag + 2L):Tt) {
      Theta[[tt]] <- Theta_fixed
      V[[tt]]     <- V_fixed
      Vinv[[tt]]  <- Vinv_fixed
    }
  }
  ...
  list(Theta = Theta, V = V, uhat = uhat, innov = innov, uhat_ahead = uhat_ahead, mylag = mylag)
}
...
innovations_mv_grad <- function(
  ...
  # Lag truncation: (matching the innovation_mv for objective function)
  #   - If lag_max is NULL, we allow up to T-1.
  #   - Otherwise, cap at min(lag_max, T-1).
  lag_cap <- if (is.null(lag_max)) max_n else min(as.integer(lag_max), max_n)
  if (lag_cap < 0L) lag_cap <- 0L

  # tolerance early stopping
  # stop early if max(abs(highest-lag Theta)) < lag_tol
  do_tol_stop <- !is.null(lag_tol)
  if (do_tol_stop) {
    lag_tol <- as.numeric(lag_tol)
    if (!is.finite(lag_tol) || lag_tol < 0) stop("lag_tol must be finite and >= 0, or NULL.")
  ...
  # add small jitter to keep matrix semi-positive
  # only added to V itself, not on gradient
  make_spd <- function(A) {
    if (symmetrize) A <- (A + t(A))/2
    A + jitter * diag(m)
  }

  # Theta[[t]] : list of coefficient matrices used to predict u_t from past innovations
  # V[[t]]     : innovation covariance matrix at time t
  # Vinv[[t]]  : V[[t]]^{-1}

  # dTheta[[t]][[j]][,,r] : derivative of Theta[[t]][[j]] wrt par_r
  # dV[[t]][,,r]          : derivative of V[[t]] wrt par_r
  Theta <- vector("list", Tt)
  V     <- vector("list", Tt)
  Vinv  <- vector("list", Tt)
  ...
  # product rule repeatedly in:     term <- term - A_n V_j A_k'
  #                                 Vn <- Vn - A V_j A'
  if (max_n >= 1L && lag_cap >= 1L) {
    for (n in 1:lag_cap) {

      Theta_n  <- vector("list", n)
      dTheta_n <- vector("list", n)

      for (k in 0:(n - 1)) {

        # term = kappa(n+1, k+1) - sum_{j=0}^{k-1} Theta_n[...] V[j+1] Theta[k+1][...]'
        term  <- getK(n + 1, k + 1)
        dterm <- getdK(n + 1, k + 1)
        ...
        if (k >= 1) {
          for (j in 0:(k - 1)) {
            A_n <- Theta_n[[n - j]]
            A_k <- Theta[[k + 1]][[k - j]]
            # term <- term - A_n V_j A_k'
            term <- term - A_n %*% V[[j + 1]] %*% t(A_k)

            # d(A_n V A_k') = (dA_n) V A_k' + A_n (dV) A_k' + A_n V (dA_k')
            dA_n <- dTheta_n[[n - j]] # m x m x p
            dA_k <- dTheta[[k + 1]][[k - j]] # m x m x p
            dV_j <- dV[[j + 1]] # m x m x p

            # for every parameter in par
            for (r in 1:p) {
              dterm[,,r] <- dterm[,,r] -
                (dA_n[,,r] %*% V[[j + 1]] %*% t(A_k)) -
                (A_n %*% dV_j[,,r] %*% t(A_k)) -
        ...
        # Theta_mat = term * Vinv[k+1]
        Theta_mat <- term %*% Vinv[[k + 1]]
        Theta_n[[n - k]] <- Theta_mat

        dTheta_mat <- array(0, dim = c(m, m, p))
        dV_k <- dV[[k + 1]]
        for (r in 1:p) {
          dTheta_mat[,,r] <- (dterm[,,r] - Theta_mat %*% dV_k[,,r]) %*% Vinv[[k + 1]]
        }
        dTheta_n[[n - k]] <- dTheta_mat
      }
      ...
      # Add jitter for numerical semi-positive difinite matrix for chol()
      Vn <- make_spd(Vn)
  ...
  # Freeze for early stop if reach pre-determined max number of lags
  if (mylag < max_n) {
    Theta_fixed <- Theta[[mylag + 1L]]
    V_fixed     <- V[[mylag + 1L]]
    Vinv_fixed  <- Vinv[[mylag + 1L]]
    dTheta_fixed <- dTheta[[mylag + 1L]]
    dV_fixed     <- dV[[mylag + 1L]]

    for (tt in (mylag + 2L):Tt) {
      Theta[[tt]] <- Theta_fixed
      V[[tt]]     <- V_fixed
      Vinv[[tt]]  <- Vinv_fixed
      dTheta[[tt]] <- dTheta_fixed
      dV[[tt]]     <- dV_fixed
    }
  }
  ...
  if (Tt >= 2L) {
    for (t in 2:Tt) {
      ...
      if (jmax >= 1L) {
        for (j in 1:jmax) {
          H   <- coefs[[j]]          # Theta_{t,j}
          dH  <- dTheta[[t]][[j]]    # dTheta_{t,j} (m x m x p)
          e_l <- innov[t - j, ]      # e_{t-j}
          de  <- dinnov[t - j, , ]   # de_{t-j} (m x p)
          for (r in 1:p) {
  ...
  list(uhat = uhat, innov = innov, duhat = duhat, dinnov = dinnov, mylag = mylag)
}
...
aop1_score_matrix <- function(par, Xw, DesignX,
  ...
  Xw <- as.matrix(Xw); DesignX <- as.matrix(DesignX)
  ...
  fit <- innovations_mv(kappa=kappa, u=u, jitter=jitter, symmetrize=symmetrize,
                        compute_innov=TRUE, lag_max=lag_max,
                        lag_tol=lag_tol, ahead=NULL)
  ...
  fitg <- innovations_mv_grad(kappa=kappa, u=u, dkappa=dkappa, du=du_arr,
                              jitter=jitter, symmetrize=symmetrize,
                              lag_max=lag_max, lag_tol=lag_tol)
