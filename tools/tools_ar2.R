...
aop2_score_matrix <- function(par, Xw, DesignX,
                              lag_max = 50L,
                              lag_tol = 1e-8,
                              jitter = 1e-10,
                              symmetrize = TRUE) {
  ...
  if (any(!is.finite(cuts)) ||
      is.unsorted(cuts, strictly = TRUE) ||
      cuts[1] <= 0) {
    stop("Invalid cutpoints.")
  }
  
  if (!is_stationary_ar2(phi1, phi2)) {
    stop("AR(2) parameters are not stationary.")
  }
  
  sig2e <- ar2_innov_var_unit(phi1, phi2)
  if (!is.finite(sig2e) || sig2e <= 0) {
    stop("Invalid AR(2) innovation variance.")
  }
  ...
  Fmat <- pnorm(outer(mst, seg, function(mu, c) c - mu))
  ...
  ## PredX = EXW + uhat
  ## du_arr = d u = - d EXW
  ## therefore d PredX = d EXW + d uhat = -du_arr + duhat
  dPredX <- -du_arr + fitg$duhat
  
  S <- matrix(0, nrow = Ts, ncol = p)
  
  for (t in seq_len(Ts)) {
    
    resid_t <- as.numeric(Xw[t, ] - Predw[t, ])
    
    for (r in seq_len(p)) {
      
      dPred_m <- as.numeric(dPredX[t, , r])
      dPred_full <- c(dPred_m, -sum(dPred_m))
      
      S[t, r] <- -2 * sum(resid_t * dPred_full)
    }
  }
...
sandwichSE_AOP2 <- function(obj, fit, Xw, DesignX,
                            lag_max = 50L,
                            lag_tol = 1e-8,
                            jitter = 1e-10,
                            symmetrize = TRUE) {
